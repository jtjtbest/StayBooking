    ...
    protected void doFilterInternal(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (jwt != null && jwtUtil.validateToken(jwt) && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if (authority != null) {
                ...
                // this token only used under security framework, not to pass to frontend.
                // purposes is we don't have to authenticate for next request within expiration time.
                usernamePasswordAuthenticationToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(httpServletRequest));
                SecurityContextHolder.getContext().setAuthentication(usernamePasswordAuthenticationToken);
                // save authorized data on securitycontextholder
                // 存在自己线程下面的, logout 后会清空
            }
        }
